...
from MAPPO_Lagrangian.mappo_lagrangian.runner.separated.base_runner import Runner
...
class MujocoRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                data = obs, share_obs_ht, share_obs_vt, rewards, dones, infos, \
            ...
            if episode % self.log_interval == 0:
                ...
                self.writter.add_scalars("train_episode_rewards", {"aver_rewards": aver_episode_rewards},
                                         total_num_steps)
                self.writter.add_scalars("train_episode_costs", {"aver_costs": aver_episode_costs}, total_num_steps)

            # eval
            if episode % 100 == 0:
                self.control(int(episode / 100))
    ...
    def insert(self, data):
        obs, share_obs_ht, share_obs_vt, rewards, dones, infos, \
    ...
    def plot_results(self, env_infos, step, e_num):
        ...
        for t in range(51):
            if t <= 4:
                ...
                speed_usl.append(-t / 50 + 1.08)
            if t >= 46:
                speed_lsl.append(0.0)
            else:
                speed_lsl.append(-t / 50 + 0.92)
        ...
        for key in ['h0', 'v1', 'v2', 'v3', 'v4', 'v5', 's1', 's2', 's3', 's4', 's5']:
            ...
            plt.xlabel('time')  # 显示x轴标签
            plt.ylabel(key)  # 显示y轴标签
            ...
            plt.savefig(os.path.join(figure_save_path, key))  # 分别命名图片
            # plt.show()
            plt.clf()
        plt.close()
        ...
        for key in ['h1', 'h2', 'h3', 'h4', 'h5', 't12', 't23', 't34', 't45']:
            ...
            control_chart.set_xlabel('time/s')
            if key in ['h1', 'h2', 'h3', 'h4', 'h5']:
                control_chart.set_ylabel(key + '/mm')
            else:
                control_chart.set_ylabel(key + '/T')
    ...
    def data_fit(eval_env_infos):
        edge = {'h0': [2.15549, 2.21369], 'h1': [1.49235, 1.52903], 'h2': [0.919962755, 0.949155565], 'h3': [0.600405128, 0.615743896],
