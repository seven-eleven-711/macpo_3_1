...
from MAPPO_Lagrangian.mappo_lagrangian.utils.separated_buffer import SeparatedReplayBuffer
...
class Runner(object):
    def __init__(self, config):
        ...
        from MAPPO_Lagrangian.mappo_lagrangian.algorithms.r_mappo.r_mappo_lagr import R_MAPPO_Lagr as TrainAlgo
        from MAPPO_Lagrangian.mappo_lagrangian.algorithms.r_mappo.algorithm.MACPPOPolicy import MACPPOPolicy as Policy
        ...
        for agent_id in range(self.num_agents):
            share_ht_space = self.envs.share_observation_space[agent_id] if self.use_centralized_V else \
                self.envs.observation_space[agent_id]

            share_vt_space = self.envs.share_policy_space[agent_id] if self.use_centralized_V else \
                self.envs.observation_space[agent_id]
            # policy network
    ...
    def compute(self):
        for agent_id in range(self.num_agents):
            ...
            if self.algorithm_name == "mappo_lagr":
                next_value = self.trainer[agent_id].policy.get_values(self.buffer[agent_id].share_ht[-1],
            ...
            if self.algorithm_name == "mappo_lagr":
                next_costs = self.trainer[agent_id].policy.get_cost_values(self.buffer[agent_id].share_vt[-1],
                                                                           self.buffer[agent_id].rnn_states_cost[-1],
                                                                           self.buffer[agent_id].masks[-1])
            else:
                next_costs = self.trainer[agent_id].policy.get_cost_values(self.buffer[agent_id].obs[-1],
                                                                           self.buffer[agent_id].rnn_states_cost[-1],
                                                                           self.buffer[agent_id].masks[-1])
    ...
    def remove_episodes(self, agent_id, del_ids):
        ...
        buffer.share_ht = np.delete(buffer.share_ht, del_ids, 1)
        buffer.share_vt = np.delete(buffer.share_vt, del_ids, 1)
