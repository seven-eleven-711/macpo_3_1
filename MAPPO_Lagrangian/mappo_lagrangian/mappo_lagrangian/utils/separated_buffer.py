...
from MAPPO_Lagrangian.mappo_lagrangian.utils.util import check, get_shape_from_obs_space, get_shape_from_act_space
...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_ht, share_obs_vt, act_space):
        ...
        share_ht_shape = get_shape_from_obs_space(share_obs_ht)
        share_vt_shape = get_shape_from_obs_space(share_obs_vt)
        ...
        self.aver_episode_costs = np.zeros((self.episode_length + 1, self.n_rollout_threads, 1),
                                           dtype=np.float32)
        self.share_ht = np.zeros((self.episode_length + 1, self.n_rollout_threads, 17+5), dtype=np.float32)
        self.share_vt = np.zeros((self.episode_length + 1, self.n_rollout_threads, 17+5), dtype=np.float32)
    ...
    def insert(self, share_obs_ht, share_obs_vt, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        self.share_ht[self.step + 1] = share_obs_ht.copy()
        self.share_vt[self.step + 1] = share_obs_vt.copy()
    ...
    def after_update(self):
        self.share_ht[0] = self.share_ht[-1].copy()
        self.share_vt[0] = self.share_vt[-1].copy()
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        """
        use proper time limits, the difference of use or not is whether you use bad_mask
        """
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None, cost_adv=None):
        ...
        share_ht = self.share_ht[:-1].reshape(-1, *self.share_ht.shape[2:])
        share_vt = self.share_vt[:-1].reshape(-1, *self.share_vt.shape[2:])
        ...
        for indices in sampler:
            ...
            share_ht_batch = share_ht[indices]
            share_vt_batch = share_vt[indices]
            ...
            if self.factor is None:
                yield share_ht_batch, share_vt_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
            else:
                if self.algo == "mappo_lagr":
                    factor_batch = factor[indices]
                    yield share_ht_batch, share_vt_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_return_batch, rnn_states_cost_batch, cost_adv_targ, aver_episode_costs
                else:
                    factor_batch = factor[indices]
                    yield share_ht_batch, share_vt_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch, cost_adv=None):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                share_obs_ht_batch.append(self.share_ht[:-1, ind])
                share_obs_vt_batch.append(self.share_vt[:-1, ind])
            ...
            share_obs_ht_batch = np.stack(share_obs_ht_batch, 1)
            share_obs_vt_batch = np.stack(share_obs_vt_batch, 1)
            ...
            share_obs_ht_batch = _flatten(T, N, share_obs_ht_batch)
            share_obs_vt_batch = _flatten(T, N, share_obs_vt_batch)
            ...
            aver_episode_costs = self.aver_episode_costs
            if self.factor is not None:
                if self.algo == "mappo_lagr" or self.algo == "IPPO_L":
                    yield share_obs_ht_batch, share_obs_vt_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_return_batch, rnn_states_cost_batch, cost_adv_targ, aver_episode_costs     # 19 value
                else:
                    yield share_obs_ht_batch, share_obs_vt_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch  # value
            else:
                yield share_obs_ht_batch, share_obs_vt_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
